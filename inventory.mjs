    ...
    if (httpMethod === 'GET') {
      ...
      for (let i = 0; i < inventory.length; i++) {
        try {
          ...
          if (!inventory[i].version) needsWriteback = true;
...
if (migratedInventory.length < inventory.length) {
  ...
  try {
    ...
    if (httpMethod === 'POST') {
      ...
      // Load existing inventory to append to, not overwrite
      const existingInventoryRaw = await readBlobData('inventory');
      const existingInventory = Array.isArray(existingInventoryRaw) ? existingInventoryRaw : [];

      // Safely migrate new items with error handling
      const newItems = [];
      if (Array.isArray(data)) {
        for (let i = 0; i < data.length; i++) {
          try {
            ...
            newItems.push(migrateItem(data[i]));
          } catch (error) {
        ...
        try {
          if (data && typeof data === 'object') {
            newItems.push(migrateItem(data));
          }
        } catch (error) {
      ...
      const combined = [...existingInventory, ...newItems];
      await writeBlobData('inventory', combined);
      return new Response(JSON.stringify(newItems.length === 1 ? newItems[0] : newItems), {
        status: 200,
        headers: { 'Content-Type': 'application/json' }
      });
    }
